...
Requires: anthropic, openai, google-genai, requests
"""
...
# Credentials are read from the explicitly exported process environment.
...
def call_anthropic(prompt: Prompt, run_number: int, model: str) -> RunResult:
    Anthropic = require_dependency("anthropic", "llm").Anthropic
...
def call_openai(prompt: Prompt, run_number: int, model: str) -> RunResult:
    OpenAI = require_dependency("openai", "llm").OpenAI
...
def call_google(prompt: Prompt, run_number: int, model: str) -> RunResult:
    genai = require_dependency("google.genai", "llm")
    types = genai.types
...
def call_perplexity(prompt: Prompt, run_number: int, model: str) -> RunResult:
    requests = require_dependency("requests", "llm")
...
def judge_classify(text: str, urls: list, brand: BrandSpec) -> tuple:
    ...
    try:
        ...
        if not m:
            print("WARNING: citation judge returned no JSON; using positional heuristic", file=sys.stderr)
        ...
        if status not in {"primary", "secondary", "none"}:
            print("WARNING: citation judge returned an invalid status; using positional heuristic", file=sys.stderr)
            return brand_position_classify(text, urls, brand)
        return status, _first_index(text, brand) if status != "none" else None
    except Exception as exc:
        print(f"WARNING: citation judge failed ({type(exc).__name__}); using positional heuristic", file=sys.stderr)
        return brand_position_classify(text, urls, brand)
...
def require_surfaces(surfaces: list, judge: bool = False) -> None:
    """Validate only selected providers before any paid calls or worker threads."""
    modules = {"anthropic": "anthropic", "openai": "openai",
               "google": "google.genai", "perplexity": "requests"}
    for surface in surfaces:
        require_dependency(modules[surface], "llm")
    if judge:
...
def write_summary_md(
    ...
    failed_count = sum(r.get("brand_cited") == "error" for r in rows)
    rows = [r for r in rows if r.get("brand_cited") != "error"]
    ...
    lines.append(f"**Failed samples excluded from shares:** {failed_count}")
    lines.append("")
...
def main() -> int:
    ...
    require_surfaces(surfaces, opts.judge)
