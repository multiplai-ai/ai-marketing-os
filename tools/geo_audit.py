...
This is the optional local execution tool for the `geo-audit` workflow. Its
scores are transparent heuristics for prioritizing review, not industry
...
Requires: requests, beautifulsoup4, textstat
...
if __package__:
    from .optional_dependencies import MissingOptionalDependency, require_dependency, run_cli
else:
    from optional_dependencies import MissingOptionalDependency, require_dependency, run_cli
import argparse
import csv
...
def fetch_html(url: str) -> tuple[str, Optional[str]]:
    ...
    requests = require_dependency("requests", "geo")
    try:
        resp = requests.get(
...
def extract_content(html: str) -> tuple[str, str, BeautifulSoup]:
    ...
    """
    BeautifulSoup = require_dependency("bs4", "geo").BeautifulSoup
    if HAS_READABILITY:
        try:
...
def require_readability_data():
    """Require explicit corpus setup before textstat can attempt a download."""
    textstat = require_dependency("textstat", "geo")
    nltk = require_dependency("nltk", "geo")
    try:
        nltk.data.find("corpora/cmudict")
    except LookupError as exc:
        raise MissingOptionalDependency(
            "The NLTK CMU pronunciation dictionary is not installed. "
            "During setup with network access, run python -m nltk.downloader cmudict "
            "using this Python environment; then rerun the audit. "
            "The audit will not download datasets implicitly."
        ) from exc
    return textstat
...
def score_readability(body_text: str) -> ContentSignal:
    ...
    textstat = require_readability_data()
...
def audit_url(
    ...
    for module in ("requests", "bs4", "textstat"):
        require_dependency(module, "geo")
    require_readability_data()
    slug = url_to_slug(url)
...
def write_audit_md(result: AuditResult, out_dir: Path) -> Path:
    ...
    lines.append(f"**Heuristic content score:** {result.overall_score}/100")
    lines.append(f"**Word count:** {result.word_count}")
    lines.append("")
    lines.append(
        "> Scores are transparent prioritization heuristics, not industry "
        "benchmarks or guarantees of search visibility or AI citations."
    )
    lines.append("")
