---
name: geo-prompt-set-builder
description: Generate a representative 50–200 prompt test set for measuring brand citation rate in generative engines, exported as a CSV that feeds directly into `geo-share-of-answers`.
---

# GEO Prompt Set Builder

Generate a representative 50–200 prompt test set for measuring brand citation rate in generative engines, exported as a CSV that feeds directly into `geo-share-of-answers`.


## Goal

Create a validated prompt set for measuring brand and competitor visibility across generative search and answer engines (ChatGPT, Claude, Gemini, Perplexity).
...
**Intent vocabulary (LOCKED — shared with `geo-share-of-answers`):**
...
Reuse supplied context and ask only for missing decisions. Confirm the audience, offer, market and intended scope before generating. Use B2B SaaS defaults only if applicable and explicitly accepted; record all assumptions.

### 2. Product Questions filter (the GEO method guide's workflow sequence)
...
Write conversational prompts, not SEO keyword strings. Include competitor and stack-context prompts where relevant.
...
- **For a broad commercial set:** shopping class ≥ agreed floor (suggested 40%). A deliberately filtered single-intent set uses its selected intent instead. If short, generate more shopping prompts before export.
...
- CSV: `{brain}/geo/prompts/<vertical>_<YYYY-MM-DD>.csv` (or a reviewed client-agnostic CSV supplied by the operator for shared templates)
...
- Handoff line: "Feed this CSV into `geo-share-of-answers` when you're ready for a baseline run."
...
**Schema lock:** first 5 columns match the input schema `geo-share-of-answers` expects. Columns 6–7 are additive metadata; the downstream skill ignores them.
...
- [ ] Segment and GTM motion reflect this client; assumptions are visible
...
- **Entity / client work:** `{brain}/geo/prompts/<vertical>_<YYYY-MM-DD>.csv`
- **Shared templates:** a reviewed client-agnostic CSV supplied by the operator

> Entity note ({brain}=organization): organization's own prompt sets historically lived at `brands/organization/geo/prompts/<vertical>_<YYYY-MM-DD>.csv` — the same `{brain}/geo/prompts/...` convention with the organization brand directory as `{brain}`.

## Quality Standard

- Prompts should resemble real buyer questions.
- Intent labels must be consistent.
- The set should be repeatable for trend tracking.

## Adaptation Notes

- Codex can generate and validate CSV files locally.
- ChatGPT can help brainstorm prompts before CSV export.
