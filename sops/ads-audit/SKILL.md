---
name: ads-audit
description: 'Full multi-platform paid advertising audit with parallel subagent delegation: assess paid media account health across Google Ads, Meta Ads, LinkedIn Ads, TikTok Ads, and Microsoft Ads, and produce per-platform and aggregate health scores plus an executive-readable action plan.'
---
...
Use when the user says "audit", "full ad check", "analyze my ads", "account health check", or "PPC audit".
...
2. **Detect business type** — analyze account signals per ads orchestrator. Classify the account context, such as SaaS, ecommerce, local service, B2B enterprise, info product, mobile app, real estate, healthcare, finance, or agency.
...
Evaluate each active platform using the relevant deep-dive subagent or reference checklist across:
...
If no exports available, audit from screenshots or manual data entry. Note missing data and confidence level.

## Scoring

Read `tools/ads-references/scoring-system.md` for full algorithm. Reference `tools/ads-references/benchmarks.md` for performance benchmarks.
...
Use platform weights and budget share to calculate platform scores and aggregate health. If data is incomplete, label scores as directional.

## Output Files

Save the audit and action plan under `{brain}/ads/audits/`:

- `{brain}/ads/audits/ads-audit-{date}.md` — Comprehensive multi-platform findings
- `{brain}/ads/audits/ads-action-plan-{date}.md` — Prioritized recommendations (Critical > High > Medium > Low)
- `{brain}/ads/audits/ads-quick-wins-{date}.md` — Items fixable in <15 minutes with high impact

Example (legacy command output names): `ADS-AUDIT-REPORT.md`, `ADS-ACTION-PLAN.md`, `ADS-QUICK-WINS.md` written to the working directory.
...
- Every finding should include evidence and impact.
- Separate account hygiene from strategic recommendations.
- Make quick wins easy to implement.

## Adaptation Notes

- Codex can parse local exports and reference files.
- ChatGPT can help workshop recommendations with stakeholders.
