---
name: geo-plan
description: Synthesize outputs from `geo-share-of-answers` and `geo-audit` into a prioritized GEO optimization plan with eng-handoff cards, a revenue impact model, an exec pitch one-pager, and a Gamma exec deck.
---

# geo-plan

Synthesize outputs from `geo-share-of-answers` and `geo-audit` into a prioritized GEO optimization plan with eng-handoff cards, a revenue impact model, an exec pitch one-pager, and a Gamma exec deck.
...
**Goal:** Translate GEO measurement and audit findings into a business-backed action plan that executives and engineering teams can act on.
...
|---|---|---|
| Audit output directory | Yes | Output of `geo-audit` (contains `diagnostic.md`, page scorecards, `data-inventory.md`) |
| SoA output directory | Yes | Output of `geo-share-of-answers` (contains `runs.csv`, `summary.md`, `trends.csv`) |
| Citation-network map | No | Output of `geo-citation-network-mapper` if available (Phase 2 skill) |
| Client name | Yes | User |
| Segment | Yes | `mid-market` \| `enterprise` \| `both` |
| GTM motion | Yes | `PLG` \| `sales-led` \| `hybrid` |
| Business inputs | Yes | ACV, win rate, conversion rates, organic traffic, team capacity (B2B SaaS defaults offered) |
...
Where `<plan-output-dir>` is `{brain}/geo/plan/<YYYY-MM-DD>/`.
...
Ask the user for business inputs. Present B2B SaaS segment defaults and let them accept or override — use defaults only when the user accepts them:
...
Also collect team capacity — it constrains phasing in Step 8.
...
- Reference the GEO method guide's workflow sequence's 7-step playbook for initiative taxonomy:
...
- **Confidence** (1–10): strength of evidence
  - 8–10: directly supported by relevant observed client evidence and a documented causal rationale
  - 5–7: supported by relevant cited research but not yet confirmed in this client's data
  - 1–4: hypothesis based on general best practice, no direct evidence
- **Effort** (1–10): 1 = trivial, 10 = months of work
  - 1–3: content-only edit, no engineering, < 1 day
  - 4–6: content + schema or config changes, 1–2 weeks
  - 7–10: architecture changes, SSR, major site restructuring

**ICE score** = (Impact × Confidence) / Effort × 10

### Step 5 — Tag SEO/GEO gate (the GEO method guide's search-quality guardrails)
...
Check each initiative against the GEO method guide's search-quality guardrails's 5 high-risk tactics:
  ...
  "initiatives": [
    {
      ...
      "description": "Restructure top 10 landing pages to place direct, definitional answers in the first 30% of content. Test whether clearer opening answers improve observed citations.",
      "goal": "B",
      "seo_geo_tag": "SEO+/GEO+",
      "ice_impact": 8,
      "ice_confidence": 8,
      ...
      "dependencies": [],
      "context": "Hypothesis: answer-first structure may improve extractability; validate with repeat measurement",
      "steps": [
        ...
        "Ensure the answer uses 'X is Y' definitional structure",
...
This is a sensitivity scenario, not a forecast or statistical confidence interval. Mention share does not establish traffic, conversion, or revenue. Record the assumed traffic mechanism separately; do not sum overlapping initiatives as incremental revenue. For non-SaaS businesses, use their actual funnel and explain where the B2B model is inapplicable. If inputs are missing, deliver the action plan and mark the revenue scenario incomplete.
...
**SEO/GEO Integration Gate (the GEO method guide's search-quality guardrails):**
...
(the GEO method guide's search-quality guardrails): rapid AI content scaling, artificial refreshing,
self-promotional listicles, prompt injection, excessive comparison pages.

**Attribution Discipline (the GEO method guide's measurement rules):**
...
- **Why** — link back to audit finding + the GEO method guide's content heuristics/04 research
- **Acceptance criteria** — checkboxes a reviewer can sign off on
- **Technical requirements** — URLs, files, pages affected
- **References** — specific framework sections in `sops/geo-audit/references/method.md`
...
- **Why Now** — 2–3 observations grounded in this client's dated audit and measurement results. Do not reuse unsourced market-growth statistics or claim that answer visibility measures pipeline.
...
- **Comparable benchmarks** — use relevant, dated sources supplied or independently verified for this engagement; label differences in audience, channel and measurement.
...
If requested, prepare a ≤10 slide exec deck from the exec one-pager. Save a local slide outline first; use a connected presentation tool only when authorized.
...
If a presentation connector is unavailable, deliver `exec-deck-outline.md` and say that no hosted deck was created. This does not block the local plan. Check the connected tool's schema rather than assuming a connector name. Keep the same approved numbers and assumptions in every format.
...
- All outputs: `{brain}/geo/plan/<YYYY-MM-DD>/`
...
- [ ] No recommendation triggers any of the GEO method guide's search-quality guardrails's 5 high-risk tactics
- [ ] Every initiative carries a the GEO method guide's search-quality guardrails SEO/GEO tag; `[GEO+ but SEO-]` items flagged and deprioritized, never recommended
- [ ] Exec one-pager ≤1 page, zero jargon without inline definition
- [ ] Initiative count ≤12
- [ ] Local deck outline delivered; hosted deck only if requested and available. Content matches the executive one-pager.
- [ ] All outputs saved to correct storage path

## Quality Standard

- Plan should be capped, prioritized, and feasible.
- Revenue model assumptions should be visible.
- Engineering cards should be specific enough to implement.
- Never recommend GEO tactics that likely harm SEO quality.

## CLI cheat sheet

Examples below use concrete paths for illustration (here `{brain}` = `clients/acme`):
...
- Codex can generate local plan files and engineering cards.
- ChatGPT is useful for sharpening the executive story and budget case.
...
- Upstream: `geo-share-of-answers` → `runs.csv`, `geo-audit` → `diagnostic.md`

> Resolve any executive-brief template from the current consumer configuration. Do not retrieve another client's example or private history as a formatting dependency.
