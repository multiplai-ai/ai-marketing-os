---
name: geo-share-of-answers
description: Run a prompt set against ChatGPT, Claude, Gemini, and Perplexity; detect citations of the brand (and competitors); output a trend-able CSV plus an exec-readable summary report.
---

# GEO Share of Answers

Run a prompt set against ChatGPT, Claude, Gemini, and Perplexity; detect citations of the brand (and competitors); output a trend-able CSV plus an exec-readable summary report.



This is the foundational measurement skill — Stage 1 of the measurement maturity ladder (the GEO method guide's measurement rules). Every other GEO skill assumes this baseline exists.
...
**Intent vocabulary (LOCKED — shared with `geo-prompt-set-builder`):**
`shopping | comparative | informational | decision | recommendation`

## When to use

- Establishing a GEO baseline for a new entity
...
|---|---|---|
| Prompt set CSV | Yes | Output of `geo-prompt-set-builder`, or any CSV matching the locked schema |
| Brands CSV (primary + competitors) | Yes | User provides (or skill generates a template via `--init-brands`) |
| Entity identifier (resolves the `{brain}` output path) | Yes | User |
| Surfaces to query | No | All 4 (anthropic, openai, google, perplexity) |
| Runs per prompt | No | 3 (handles AI response stochasticity) |
| Citation classifier mode | Yes — **ASK** | A: positional heuristic (default, fast/free) **or** B: LLM-as-judge (slower, ~$0.50/100 rows, accuracy must be evaluated on a reviewed sample) |
...
- `{brain}/geo/share-of-answers/<YYYY-MM-DD>/`

> Example: with `{brain}` = `clients/acme`, a run lands in `clients/acme/geo/share-of-answers/2026-04-16/`.
...
- [ ] User has a valid prompts CSV (`geo-prompt-set-builder` output, or hand-built matching the schema)
...
- Never run paid API calls without user confirmation.
- Preserve prompt set and classifier settings for trendability.
- Report shopping-intent share separately from overall mention share.
...
API measurements reflect these configured API models, not the consumer ChatGPT or Claude apps or Google AI Mode. Record model IDs, search settings, errors and dates; do not compare incompatible runs as a trend. Cost estimates are approximate and are not hard billing limits.
...
Every `summary.md` ends with the 5 fake-case-study warnings (the GEO method guide's measurement rules) and the SEO/GEO investment gate (the GEO method guide's search-quality guardrails). These are not optional — they exist to discipline how the data is interpreted. Don't strip them, don't summarize them when surfacing the report inline.

## CLI cheat sheet (examples — `{brain}` shown literally; substitute the resolved entity path)

```bash
# Generate brands template
python3 tools/geo_share_of_answers.py --init-brands {brain}/geo/brands.csv

# Quick test (5 prompts, 1 run, no confirmation)
python3 tools/geo_share_of_answers.py \
    --prompts <approved-prompts.csv> \
    --brands  {brain}/geo/brands.csv \
    --output  .tmp/geo/test/ \
    --limit 5 --runs-per-prompt 1 --no-confirm

# Production baseline (full set, 3 runs, confirmation gate)
python3 tools/geo_share_of_answers.py \
    --prompts {brain}/geo/prompts.csv \
    --brands  {brain}/geo/brands.csv \
    --output  {brain}/geo/share-of-answers/2026-04-16/ \
    --runs-per-prompt 3
...
- Codex is best for running local tools and managing CSV outputs.
- ChatGPT can review summaries and help interpret findings.
...
- Upstream skill: `geo-prompt-set-builder` → produces the prompt CSV input
