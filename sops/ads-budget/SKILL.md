---
name: ads-budget
description: 'Budget allocation and bidding strategy review across all ad platforms: evaluates spend distribution, bidding strategy appropriateness, scaling readiness, and identifies campaigns to kill or scale using the 70/20/10 rule, 3x Kill Rule, and 20% scaling rule.'
---
...
Evaluate whether spend and bidding are allocated to the right channels, campaigns, and learning stages.

Use when the user says "budget allocation", "bidding strategy", "ad spend", "ROAS target", "media budget", or "scaling".

## Process

1. Collect budget and performance data across all active platforms (platform, campaign, objective, spend, conversions, revenue, CPA, ROAS, and budget limits)
2. Read `tools/ads-references/budget-allocation.md` for allocation framework
3. Read `tools/ads-references/bidding-strategies.md` for strategy decision trees
4. Read `tools/ads-references/benchmarks.md` for CPC/CPA benchmarks
5. Read `tools/ads-references/scoring-system.md` for health score algorithm
6. Evaluate budget allocation, bidding strategy, and scaling readiness
   - Classify spend as proven, scaling, or testing; flag overinvestment in low-signal tests and underinvestment in proven channels
   - Check whether each campaign has enough daily or monthly budget to exit learning and generate meaningful data
   - Evaluate whether bidding matches conversion volume, tracking quality, and goal maturity
7. Generate recommendations with kill list and scale list (scale / hold / fix before scaling / pause or kill)
8. Define next budget state: show proposed budget allocation with rationale and expected impact
...
Save the full report to `{brain}/ads/budget/ads-budget-review-{date}.md` (legacy rich-copy name: `BUDGET-STRATEGY-REPORT.md`), containing:
...
- Proposed next budget state with rationale and expected impact
- Quick Wins for immediate budget optimization

## Quality Standard

- Separate poor performance from insufficient data.
- Do not recommend scaling before tracking and learning conditions are met.
- Tie budget moves to business goals, not vanity metrics.

## Adaptation Notes

- Codex can analyze exported spend tables.
- ChatGPT can help explain tradeoffs to finance or leadership.
