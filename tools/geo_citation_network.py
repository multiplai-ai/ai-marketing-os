...
Requires: anthropic, openai, google-genai, requests, tldextract
"""
...
def extract_domain(url: str) -> tuple:
    ...
    if _tld_extractor is None:
        tldextract = require_dependency("tldextract", "geo")
...
def main() -> int:
    ...
    classification.add_argument("--llm-categorize", dest="no_llm_categorize", action="store_false", help="Opt in to paid Claude classification of unknown domains")
    classification.add_argument("--no-llm-categorize", dest="no_llm_categorize", action="store_true", help="Use only offline domain rules (default)")
    parser.set_defaults(no_llm_categorize=True)
    ...
    require_dependency("tldextract", "geo")
    if not opts.no_llm_categorize:
