...
def parse_audit_dir(audit_dir: Path) -> dict[str, Any]:
    ...
    for md_file in sorted(audit_dir.glob("*_audit.md")):
        ...
        m = re.search(r"(?:Overall|Heuristic) content score:\*?\*?\s*(\d+\.?\d*)/100", text)
        if m:
...
def scaffold_exec_one_pager(
    ...
    lines.extend([
        ...
        "## Why Now",
        "",
        "<!-- 2-3 observations grounded in this client's dated evidence; cite the sources -->",
        "",
        "## Top 3 Initiatives",
        "",
    ])
...
def scaffold_business_case(output_dir: Path, client: str) -> Path:
    ...
    lines = [
        ...
        "",
        "<!-- Add relevant verified benchmarks with date, source and comparability limits. Leave unassessed if absent. -->",
        "",
        "## FAQ",
        "",
